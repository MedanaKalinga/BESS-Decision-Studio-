  ...
  const reviewContent = <Stack spacing={2}>
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "repeat(2,minmax(0,1fr))" }, gap: 1.5 }}>
      <ReviewSection title="Battery Alternatives" onEdit={() => editReviewSection(0)}>
        <Typography variant="body2">{configuration.batteries.filter((entry) => entry.enabled).map((entry) => batteryDisplayName(entry.battery.name)).join(", ")}</Typography>
      </ReviewSection>
              ...
              >
                ...
                {summaryResults.flatMap((result) => [
                  <Box key={`${result.battery_name}-name`} role="rowheader" sx={{ p: 1.2, bgcolor: "#0D1D2D" }}>
                    <Typography variant="body2" sx={{ fontWeight: 850 }}>{batteryDisplayName(result.battery_name)}</Typography>
                    <Chip
