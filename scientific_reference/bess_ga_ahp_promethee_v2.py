...
# - DoD sensitivity                               (minimize)
# - annual O&M cost                               (minimize)
...
# ============================================================
# AHP is used only to calculate the relative importance of the six criteria.
...
AHP_CRITERIA_LABELS = {
    ...
    "bess_om_cost_annual_Rs": "Annual O&M cost",
    "warranty_years": "Warranty period",
}

# Reference weights requested for the study. These are used only for comparison
# in the AHP output table. PROMETHEE II uses the weights calculated from the
# pairwise-comparison matrix below.
#AHP_REFERENCE_WEIGHTS = {
 #   "total_annual_cost_Rs": 0.35,
  #  "cycle_based_life_years": 0.27,
   # "round_trip_efficiency": 0.08,
    #"dod_sensitivity_exponent": 0.12,
   # "bess_om_cost_annual_Rs": 0.11,
   # "warranty_years": 0.08,
#}
...
# This matrix gives weights close to the requested 0.35, 0.30, 0.10, 0.08,
# 0.10 and 0.07 vector while maintaining a low consistency ratio.
...
# ============================================================
# IMPORTANT:
# The DoD-sensitivity and warranty values below are placeholders. Replace them
...
PROMETHEE_EXTRA_CRITERIA = {
    1: {"weight_density_kg_per_kwh": 8.5, "warranty_years": 5.0},
    2: {"weight_density_kg_per_kwh": 8.0, "warranty_years": 7.0},
    3: {"weight_density_kg_per_kwh": 7.5, "warranty_years": 10.0},
    4: {"weight_density_kg_per_kwh": 7.0, "warranty_years": 12.0},
}
...
CRITERIA_TYPES = {
    ...
    "bess_om_cost_annual_Rs": "min",
    "warranty_years": "max",
}

# PROMETHEE Type III V-shape preference:
# q = 0, therefore there is no indifference zone
# p = full observed criterion range
PROMETHEE_Q_RANGE_FRACTION = 0.0
PROMETHEE_P_RANGE_FRACTION = 1.0
...
def preference_linear(difference, q, p):
    """
    PROMETHEE Type III V-shape preference when q = 0.
    """
    if p <= q:
        raise ValueError(
...
def build_decision_matrix(fixed_summary_df):
    """Create the six-criterion PROMETHEE decision matrix."""
    ...
    for _, stage1_row in fixed_summary_df.iterrows():
        ...
        rows.append({
            ...
            "bess_om_cost_annual_Rs": float(stage1_row["bess_om_cost_annual_Rs"]),
